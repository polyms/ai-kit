...
name: ui-kit
description: >-
  Build UI with @polyms/ui-kit — compound components, semantic theming, CSS utilities, responsive product UI.
  Entry skill: open routing table before coding. Form in overlay → field.md + modal.md.
---

# @polyms/ui-kit

`@polyms/ui-kit` **is** the design system — no shadcn/ui, Fluent, Carbon, or parallel libraries for the same surfaces.
...
| ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| migrate, migration, redesign, refresh, modernise, legacy, overhaul, re-skin, preserve brand, swap to ui-kit, existing screen, visual debt | [redesign.md](redesign.md)                                                                                                                                                                                               |
...
| Avatar, Breadcrumb, Toc, Accordion, FAQ, expand/collapse                                                                                  | [display.md](display.md#avatar) · [navigation.md](navigation.md#breadcrumb) · [navigation.md](navigation.md#toc) · [display.md](display.md#accordion) · [display.md](display.md#collapsible)                             |
...
| compound API, public surface, compound slots, useRender wrapper, mergeProps, extending primitive                                          | [components.md](components.md)                                                                                                                                                                                           |
...
| jargon, useRender, mergeProps, compound tree, item-*, ghost style, Container, barrel import                                               | [glossary.md](glossary.md)                                                                                                                                                                                               |
...
1. Barrel import only — `import { … } from '@polyms/ui-kit'`; trust `index.d.ts` over memory.
...
**Frequent mistakes:** conversational `Field.Label` ([quality.md#field-label-copy](quality.md#field-label-copy)); `Modal.Trigger` / footer `Modal.Close` / `Offcanvas.Trigger` / body `Offcanvas.Close` via `render={<Button … />}` ([button.md](button.md#compose-button-through-render), [modal.md](modal.md)); footer `Modal.Close` without `variant` (ghost); `autoFocus` on footer CTA when modal has form inputs — use first input instead ([modal.md#modal](modal.md#modal)); no invented `content` prop on `Button`; redundant `type='button'` on `<Button>` ([button.md](button.md#type--omit-the-default)).
