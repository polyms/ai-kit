---
name: devops
description: Deploy, CI, and infra — symptom → fix via Knowledge retrieval. Invoke with /devops, deploy failure, Vercel build failed, CI broken, sửa deploy, lỗi build, infra fix, or use devops-agent for long incident sessions.
...
**Boundary vs `/dev`:** deploy-aware `/dev` may touch infra config inside a feature slice; **`/devops`** owns operational incidents end-to-end. Hand off app feature work to `/dev`.
...
**Completion criterion:** Symptom stated; stack axes noted when manifest exists; app deploy guide located when applicable.
